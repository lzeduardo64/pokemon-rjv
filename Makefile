...
TITLE       := POKEMON RJVZ
GAME_CODE   := BPEE # Emulators needs this to be valid
MAKER_CODE  := 69 # nice
REVISION    := 0
MODERN      ?= 0
...
ROM_NAME := pokerjv.gba
ELF_NAME := $(ROM_NAME:.gba=.elf)
MAP_NAME := $(ROM_NAME:.gba=.map)
OBJ_DIR_NAME := build/rjv

MODERN_ROM_NAME := pokerjv_modern.gba
...
.PHONY: all rom clean compare tidy tools mostlyclean clean-tools $(TOOLDIRS) berry_fix libagbsyscall modern debugging debugging_modern tidymodern tidynonmodern
